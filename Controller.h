...
/**
* 게임에 출력될 음악 헤더파일과 정의.
* BGM은 예를들어 "C:\\Users\\User\\Desktop\\tetris\\음악명" 경로를 문자열로 저장.
* 경로는 \ 가 아닌, \\ 해야함.
* #define으로 BGM 정의하고 main함수에서 사용 가능.
* main()문에 BGM 부분 직접 경로를 작성 해도 됨.
*/
#include <mmsystem.h>
#pragma comment(lib, "winmm.lib")
// 음원은 저작권의 문제로 따로 제공 X
// #define BGM "C:\\Users\\User\\Desktop\\tetris\\음악명.wav" // WAV Full path
...
/**
* 새로운 블록인가? (flag 느낌)
* 새로운 블록 == true라면 새로운 블록을 false 처리 한 후에
...
/**
* 회전 기능의 방향키를 입력 받을 시
* turn(현재 블록의 회전 상태 0,1,2,3 중 예를 들어 turn == 0번째 회전 상태(초기블럭이라면)
...
* 현재 콘솔창의 커서값 Cursor에서 한 칸 내려갔을 때
...
* 현재 좌표에 출력된 블록을 블록의 바닥으로 인식시킴. blockFixed(int shape, int rotate)
*/
...
/**
* 보드의 특정 가로줄이 블록으로 저장되어있는가? isMaxLine()
* yes? -> deleteLine()
*/
boolean IsMaxLine();
void deleteLine();
