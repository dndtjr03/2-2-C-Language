...
// 블록을 삭제하는 것이 아니라 공백으로 출력하낟. 삭제하는 느낌.
...
void previewBlock(int shape, int rotate)
{
	...
	while (!IsCollision(curShape, turn, Pos.X, Pos.Y + 1)) {
		++Pos.Y;
		if (IsCollision(curShape, turn, Pos.X, Pos.Y) == true) {
...
/**
* 현재 보드 안에서 출력되는 특정한 블록에 대해서는 블록의 색을 blockType에 따라서 지정하면 되었는데,
* 초기버전에 블록이 바닥에 닿아서 쌓일 때 보드 안에 추가된 블록을 2로 정했는데,
* 그러고 나니까 쌓인 블록은 전부 같은 색이 되버려서
* 특정 blockType를 매개변수로 받은 인자값 shape에 3씩 더해서 보드에 저장.
* 이렇게 하면
* 보드 벽 = 1, 보드 안 빈 공간 = 0, 보드 안 내려오고 있는 블록 = 2
* bottom에 닿은 블록은 내려오고 있는 블록의 종류에 따라서 0 ~ 6까지라면 +3을 더해서
	...
	for (int y = BoardHeight - 2; y > 0; y--) {
		...
		for (int x = 1; x < BoardWidth - 1; x++) {
			...
			if (count >= 12) {
				int height = y;
				score += 1000;
				// 여기는 for문이 반복되는데, 블록이 사라지는 이펙트를 꾸며봤음.
				// Sleep메서드를 사용하여 간단하게.
				for (int x = 1; x < BoardWidth - 1; x++) {
					board[height][x] = 0;
					if (board[height][x] == 0) {
				...
				for (height = y; height > 0; height--) {
					for (int x = 1; x < BoardWidth - 1; x++) {
						if (board[height][x] >= 3 && board[height][x] >= 9) {
							GotoXY(x * 2 + BoardX, height + BoardY);
							/**
							* 여기에 - 3을 한 이유는 원래 블록의 모양이 4차원 배열인데 첫번째 인덱스에 따라서 블록의 유형이 달라짐.
							* 0부터 6까지 총 7개인데, blockFixed()메서드의 코드 리뷰에서 설명했듯이
							* 바닥에 쌓이게 된 블록을 색칠하기 쉽게 표현하려고 +3씩 더했는데, (3 ~ 9 : 0 ~ 6에서 +3해서)
							* colorRetention() 이 함수는 0부터 6까지 색깔을 미리 정한 상태여서 -3.
							* colorRetention() 함수의 switch문을 3부터 시작해도 되긴 함.
							*/
...
void GameProcess()
{
	...
	InGameFrame();
	ShowNextBlock();
	// 방향키를 계속해서 알 수 있도록 while(){}문과 _kbhit(), _getch()메서드 사용
	while (true) {
		if (IsNextBlock == true) {
			IsNextBlock = false;
			DeleteNextBlock();
		...
		// 바닥과 닿았는지, 현재 좌표에서 + 1한 y값을 IsCollision검사 후 바닥과 닿았다면 bockFixed() 수행,
		if (!IsCollision(curShape, turn, Cursor.X, Cursor.Y + 1))
			++Cursor.Y;
